...
struct ObserverHandle::Impl {
    std::shared_ptr<void>  keep_alive;  ///< keeps Entry alive as long as handle exists
    std::function<void()>  deactivate;  ///< marks Entry::active = false; nullptr after release()
};
...
ObserverHandle ObserverHandle::from_parts(std::shared_ptr<void>  keep_alive,
