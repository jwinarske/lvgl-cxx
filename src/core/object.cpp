...
struct EventHandle::Impl {
  ...
  Object* object = nullptr;  // safe to dereference only if token live
...
auto Object::children() const noexcept -> std::ranges::subrange<
...
int32_t Object::width() const noexcept {
  return impl_->w;
}
int32_t Object::height() const noexcept {
  return impl_->h;
}
...
Object& Object::align(Align a, int32_t x_ofs, int32_t y_ofs) noexcept {
  ...
  switch (a) {
    ...
    case Align::RightMid:
      ...
      break;
    // ── Outside parent ───────────────────────────────────────────────────
    case Align::OutTopLeft:
      nx = 0;
      ny = -h;
      break;
    case Align::OutTopMid:
      nx = (pw - w) / 2;
      ny = -h;
      break;
    case Align::OutTopRight:
      nx = pw - w;
      ny = -h;
      break;
    case Align::OutBottomLeft:
      nx = 0;
      ny = ph;
      break;
    case Align::OutBottomMid:
      nx = (pw - w) / 2;
      ny = ph;
      break;
    case Align::OutBottomRight:
      nx = pw - w;
      ny = ph;
      break;
    case Align::OutLeftTop:
      nx = -w;
      ny = 0;
      break;
    case Align::OutLeftMid:
      nx = -w;
      ny = (ph - h) / 2;
      break;
    case Align::OutLeftBottom:
      nx = -w;
      ny = ph - h;
      break;
    case Align::OutRightTop:
      nx = pw;
      ny = 0;
      break;
    case Align::OutRightMid:
      nx = pw;
      ny = (ph - h) / 2;
      break;
    case Align::OutRightBottom:
      nx = pw;
      ny = ph - h;
      break;
    default:
...
int32_t Object::scroll_x() const noexcept {
  return impl_->scroll_x;
}
int32_t Object::scroll_y() const noexcept {
...
void Object::remove_event(EventHandle& h) {
  ...
  if (impl_->dispatch_depth == 0) {
    std::erase_if(impl_->handlers, [](const auto& en) { return !en.active; });
  }
}
...
void Object::invalidate() noexcept {
  invalidate_area(bounds());
}

void Object::invalidate_area(Area /*a*/) noexcept {
  // Dirty-region registration will be connected to Display in Phase 4.
  // For now, invalidate is a no-op at the object level; Display::refresh()
  // handles full/partial redraw based on its own dirty list.
}
