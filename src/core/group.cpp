...
struct Group::Impl {
  std::vector<Object*>
      members;           // non-owning pointers; objects must outlive Group
...
namespace {
Group*& default_group_ref() noexcept {
  static Group* instance = nullptr;
  return instance;
}
}  // namespace
...
Group::~Group() {
  if (default_group_ref() == this)
    default_group_ref() = nullptr;
...
void Group::remove_all() {
  ...
  if (impl_->focused_idx >= 0) {
    impl_->members[static_cast<std::size_t>(impl_->focused_idx)]->remove_state(
        ObjState::Focused);
  }
  impl_->members.clear();
  impl_->focused_idx = -1;
...
void Group::swap(Object& a, Object& b) {
  ...
  if (it_a == impl_->members.end() || it_b == impl_->members.end())
    return;
...
void Group::do_focus(int new_idx) noexcept {
  ...
  const bool hit_edge =
      !impl_->wrap && (at_start || at_end) && new_idx != impl_->focused_idx;

  // Remove Focused state from the previously focused object
  if (impl_->focused_idx >= 0 && impl_->focused_idx < n) {
    impl_->members[static_cast<std::size_t>(impl_->focused_idx)]->remove_state(
        ObjState::Focused);
    impl_->members[static_cast<std::size_t>(impl_->focused_idx)]->send_event(
        EventCode::Defocused, nullptr);
  }
  ...
  if (impl_->on_focus_change)
    impl_->on_focus_change(*this);
  if (hit_edge && impl_->on_edge)
    impl_->on_edge(*this, at_start);
}

void Group::focus_next() noexcept {
  if (impl_->freeze || impl_->members.empty())
    return;
...
void Group::focus(Object& obj) noexcept {
  ...
  if (it == impl_->members.end())
    return;
  const int idx = static_cast<int>(std::distance(impl_->members.begin(), it));
  do_focus(idx);
}
...
void Group::set_refocus_policy(RefocusPolicy p) noexcept {
  impl_->policy = p;
}
void Group::set_editing(bool en) noexcept {
  impl_->editing = en;
}
void Group::set_wrap(bool en) noexcept {
  impl_->wrap = en;
}

RefocusPolicy Group::refocus_policy() const noexcept {
  return impl_->policy;
}
bool Group::is_editing() const noexcept {
  return impl_->editing;
}
bool Group::wrap() const noexcept {
  return impl_->wrap;
}
...
void Group::set_as_default() {
  default_group_ref() = this;
}
Group* Group::default_group() noexcept {
  return default_group_ref();
}
