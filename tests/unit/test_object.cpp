...
//
// Coverage targets (Phase 1 checklist from
// docs/design/11-implementation-plan.md):
...
TEST(ObjectTree, ChildrenEmptyRange) {
  Screen root;
  int count = 0;
  for ([[maybe_unused]] auto& c : root.children())
    ++count;
  EXPECT_EQ(count, 0);
}
...
TEST(ObjectTree, ChildrenConstRange) {
  ...
  const Object& croot = root;
  int count = 0;
  for ([[maybe_unused]] auto& c : croot.children())
    ++count;
  EXPECT_EQ(count, 1);
}
...
TEST(ObjectTree, RemoveChildByIndexOutOfRangeThrows) {
  Screen root;
  EXPECT_THROW(root.remove_child(static_cast<std::size_t>(0)),
               std::out_of_range);
}
...
static std::pair<int32_t, int32_t> aligned_pos(int32_t parent_w,
                                               int32_t parent_h,
                                               int32_t child_w,
                                               int32_t child_h,
                                               Align a,
...
TEST(ObjectLayout, MarkLayoutDirtyChains) {
  Screen root;
  // Just verify chaining works — no observable state yet (Phase 4 adds
  // rendering)
