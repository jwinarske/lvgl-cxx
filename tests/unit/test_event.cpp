...
TEST(EventBubble, PropagatesFromChildToParentWhenFlagSet) {
    ...
    auto hc = child.on(EventCode::Clicked, [&](Event&) {
        fire_order.push_back(&child);
    });
...
TEST(EventBubble, StopBubblingPreventsPropagation) {
    ...
    int parent_count = 0;
    auto hc = child.on(EventCode::Clicked, [](Event& e) {
        e.stop_bubbling();
    });
...
TEST(EventTrickle, PropagatesFromParentToChildrenWhenFlagSet) {
    ...
    std::vector<Object*> fire_order;
    auto hp  = parent.on(EventCode::ValueChanged, [&](Event&) {
        fire_order.push_back(&parent);
    });
...
TEST(EventKey, KeyExtractedFromParam) {
    ...
    auto h = w.on(EventCode::Key, [&](Event& e) {
        received_key = e.key();
    });
