...
//
// Coverage targets (Phase 2 checklist from
...
TEST(Transition, ForPropsChaining) {
  Transition t;
  t.for_props<prop::BgColor>().for_props<prop::TextColor>();
  EXPECT_TRUE(t.affects(prop::BgColor::id));
...
TEST(StyleSheet, CascadeMultipleStatesMostSpecificWins) {
  ...
  ss.add(s_focused_pressed,
         {Part::Main, ObjState::Focused | ObjState::Pressed});
...
TEST(ObjectStyle, ResolveStyleWithPartSelector) {
  ...
  auto v_sb =
      w.resolve_style(prop::BgColor{}, ObjState::Default, Part::Scrollbar);
  ASSERT_TRUE(v_sb.has_value());
