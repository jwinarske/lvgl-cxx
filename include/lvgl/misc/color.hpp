...
#include <compare>
#include <cstdint>
// lvgl_config.hpp is generated by meson into the build-root and its directory
// is added to the include path via generated_conf_dep.  Use a non-relative
// include so the same source works whether built in-tree or out-of-tree.
...
// ───────────────────────────────────── Internal storage is always 32-bit ARGB;
// the renderer converts to the active pixel format at draw time.
struct Color {
  uint8_t r = 0, g = 0, b = 0, a = 255;

  constexpr Color() noexcept = default;
  constexpr Color(uint8_t cr, uint8_t cg, uint8_t cb, uint8_t ca = 255) noexcept
      : r(cr), g(cg), b(cb), a(ca) {}
  ...
  [[nodiscard]] constexpr Color mix(Color other, uint8_t ratio) const noexcept {
    // ratio 0 → this color, ratio 255 → other color
    auto lerp8 = [](uint8_t va, uint8_t vb, uint8_t t) -> uint8_t {
      return static_cast<uint8_t>(
          va + (static_cast<int32_t>(vb) - static_cast<int32_t>(va)) *
                   static_cast<int32_t>(t) / 255);
    };
    return {lerp8(r, other.r, ratio), lerp8(g, other.g, ratio),
            lerp8(b, other.b, ratio), lerp8(a, other.a, ratio)};
  }
