...
struct Area {
    ...
    [[nodiscard]] static constexpr Area from_size(
            int32_t x, int32_t y, int32_t w, int32_t h) noexcept {
        return {x, y, x + w - 1, y + h - 1};
    }

    [[nodiscard]] constexpr int32_t width()  const noexcept { return x2 - x1 + 1; }
    [[nodiscard]] constexpr int32_t height() const noexcept { return y2 - y1 + 1; }
    ...
    [[nodiscard]] constexpr Area intersect(const Area& o) const noexcept {
        return {std::max(x1, o.x1), std::max(y1, o.y1),
                std::min(x2, o.x2), std::min(y2, o.y2)};
    }
