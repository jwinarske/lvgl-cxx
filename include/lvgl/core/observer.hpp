...
// ── ObserverHandle
// ──────────────────────────────────────────────────────────── RAII
// subscription token.  Destroying the handle unsubscribes automatically. Call
// release() to detach from RAII (subscription lives until Subject destroyed).
...
// ── Subject<T>
// ──────────────────────────────────────────────────────────────── Holds a
// value; registered observers are notified whenever it changes. Mirrors
// lv_subject_t in LVGL v9.5.0 (src/core/lv_obj_property.c).
template <typename T>
class Subject {
 public:
  explicit Subject(T initial = T{}) : value_(std::move(initial)) {}
  ...
  [[nodiscard]] ObserverHandle subscribe(UniqueFunction<void(const T&)> fn) {
 ...
 private:
  struct Entry {
    ...
    bool active = true;
    explicit Entry(UniqueFunction<void(const T&)> f) : fn(std::move(f)) {}
  };
  ...
  void notify() {
    ...
    std::erase_if(subscribers_, [](auto& sp) { return !sp || !sp->active; });
  }

  ObserverHandle make_handle(std::shared_ptr<Entry> entry) {
    return ObserverHandle::from_parts(
        entry, [e = entry]() noexcept { e->active = false; });
  }
