...
// ── Style value storage
// ─────────────────────────────────────────────────────── Variant mirrors
// lv_style_value_t plus a Transition descriptor.
using StyleValue = std::variant<std::monostate,  // property is not set
                                ...
                                Transition    // animation descriptor for
                                              // state-change transitions
                                >;

// ── Property tag types
// ──────────────────────────────────────────────────────── Each tag encodes its
// expected value type via `value_type` and a stable integer `id`.  New
// properties are added here as phases are implemented.
namespace prop {
...
struct ColorFilterOpa {
  using value_type = int32_t;
  static constexpr uint16_t id = 82;
};
struct AnimDuration {
  using value_type = int32_t;
  static constexpr uint16_t id = 83;
};
struct BlendMode {
  using value_type = int32_t;
  static constexpr uint16_t id = 84;
};
struct Transform {
  using value_type = int32_t;
  static constexpr uint16_t id = 85;
};
struct TransformWidth {
  using value_type = int32_t;
  static constexpr uint16_t id = 86;
};
struct TransformHeight {
  using value_type = int32_t;
  static constexpr uint16_t id = 87;
};
struct TransformScaleX {
  using value_type = int32_t;
  static constexpr uint16_t id = 88;
};
struct TransformScaleY {
  using value_type = int32_t;
  static constexpr uint16_t id = 89;
};
struct TransformRotation {
  using value_type = int32_t;
  static constexpr uint16_t id = 90;
};
struct TransformPivotX {
  using value_type = int32_t;
  static constexpr uint16_t id = 91;
};
struct TransformPivotY {
  using value_type = int32_t;
  static constexpr uint16_t id = 92;
};
...
class Style {
 public:
  ...
  Style& set(P /*tag*/, P::value_type value) {
  ...
  // Example: style.set_transition(Transition{300,
  // 0}.for_props<prop::BgColor>())
...
// ── StyleSheet
// ──────────────────────────────────────────────────────────────── Per-object
// ordered list of (Style*, StyleSelector) pairs. Mirrors LVGL v9.5.0's
// lv_obj_style_t list in lv_obj_style.c.
//
...
class StyleSheet {
 public:
  ...
  [[nodiscard]] StyleValue resolve(uint16_t prop_id,
                                   Part part,
                                   ObjState state) const noexcept;

  // Typed resolve — returns nullopt if not set.
  template <StyleProperty P>
  [[nodiscard]] std::optional<typename P::value_type> resolve(
      P /*tag*/,
      const ObjState state = ObjState::Default,
      const Part part = Part::Main) const noexcept {
