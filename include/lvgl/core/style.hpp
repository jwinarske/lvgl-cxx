...
// ── Style selector ────────────────────────────────────────────────────────────
// Encodes the (Part, ObjState) pair that scopes a style rule.
// Mirrors lv_style_selector_t but as a value type.
...
// ── Property tag types ────────────────────────────────────────────────────────
// Each tag encodes its expected value type.
// New properties are added here as the library is implemented.
namespace prop {
...
private:
    // std::flat_map for sorted, cache-friendly storage
    std::flat_map<uint16_t, StyleValue> props_;
};

} // namespace lv

// ObjState is defined in object.hpp; include after to avoid circular deps
// (object.hpp → event.hpp → style.hpp → object.hpp circular).
// Translation units must include object.hpp themselves.
