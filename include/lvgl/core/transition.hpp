...
//
// Style-transition descriptor — mirrors lv_style_transition_dsc_t in LVGL
// v9.5.0 (src/core/lv_obj_style.c).  Specifies how a style property value
// animates when the object's state changes.
//
...
// ────────────────────────────────────────────────────── Maps normalised
// progress t ∈ [0, 1] → eased progress ∈ [0, 1]. C++23 noexcept is part of the
// function type.
using TransitionEasingFn = float (*)(float t) noexcept;

// ── Built-in easing helpers
// ─────────────────────────────────────────────────── Correspond to LVGL's
// lv_anim_path_* functions.
namespace Easing {
...
// ── HasPropId concept
// ───────────────────────────────────────────────────────── Minimal requirement
// for for_props(): the type only needs a uint16_t id. This avoids pulling in
// StyleProperty (which is defined in style.hpp).
template <typename P>
...
// ── Transition
// ──────────────────────────────────────────────────────────────── Stored as a
// style property value (prop::PropTransition) on a Style object. When the
// framework detects a state change, it looks up the Transition for each
// property that changed and schedules an animated interpolation.
struct Transition {
