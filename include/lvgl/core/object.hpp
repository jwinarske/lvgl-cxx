...
//    on a complete Object type.  It auto-nullifies when the target is
//    destroyed.
...
class Object {
 public:
  ...
  [[nodiscard]] auto children() const noexcept -> std::ranges::subrange<
  ...
  Object& align_to(const Object& base,
                   Align a,
                   int32_t x_ofs = 0,
  ...
  [[nodiscard]] std::optional<typename P::value_type> resolve_style(
      P /*tag*/,
...
// ── ObjectRef<T>
// ────────────────────────────────────────────────────────────── Non-owning
// handle that auto-nullifies when the target Object is destroyed. Defined here
// — AFTER Object — so Object is complete and validity_token() can be called
// safely.
template <typename T>
class ObjectRef {
 public:
